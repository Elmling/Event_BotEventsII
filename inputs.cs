...
if(!$BotEventsLoaded)
{
	...
	RegisterInputEvent(fxDtsBrick, "onBotSpawn", "Self fxDtsBrick" TAB "Bot Player");
...
package BotEvents
{
	...
	function fxDTSbrick::botVehicleSpawn(%this)
	{
		if(isObject(%obj = %this.vehicle))
		{
			if(%obj.getClassName() $= "AIplayer" && isObject(%brick = %obj.spawnBrick))
			{
				...
				%this.processInputEvent("onBotSpawn", %this.getGroup().client);
			}
		}
	}
	...
	// function getTrustLevel(%a, %b) //to prevent crashes if bots should destroy their own bricks
	// {
