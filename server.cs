...
datablock playerData(botStandardArmor : playerStandardArmor)
{
	uiName = "Standard Bot";
	rideable = true;
};
...
function serverCmdApplyAv(%client)
{
	...
	if(isObject(%brick = %client.wrenchBrick))
	{
		...
		for(%i = 0; %i < %brick.numEvents; %i++)
		{
			...
			if(%brick.eventInput[%i] == "OnBotSpawn")// && %brick.eventOutput[%i] == $EditAppearance)
			{
				continue;
			}
		...
		%blanks = 9;
		serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 1, "ALL", 0);
		if(%client.hat)
		{
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $Hat[%client.hat], getClosestPaintColor(%client.hatColor));
			%blanks++;
			if(%client.accent)
			{
				serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, getWord($AccentsAllowed[$hat[%client.hat]], %client.accent), getClosestPaintColor(%client.accentColor));
			%blanks++;
			}
		}
		serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, "headSkin", getClosestPaintColor(%client.headColor));
		serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $Chest[%client.chest], getClosestPaintColor(%client.chestColor));
		if(%client.secondPack)
		{
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $SecondPack[%client.secondpack], getClosestPaintColor(%client.secondPackColor));
			%blanks++;
		}
		if(%client.pack)
		{
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $Pack[%client.pack], getClosestPaintColor(%client.packColor));
			%blanks++;
		}
		serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $LArm[%client.larm], getClosestPaintColor(%client.larmColor));
		serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $RArm[%client.rarm], getClosestPaintColor(%client.rarmColor));
		serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $LHand[%client.lhand], getClosestPaintColor(%client.lhandColor));
		serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $RHand[%client.rhand], getClosestPaintColor(%client.rhandColor));
		serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $hip[%client.hip], getClosestPaintColor(%client.hipColor));
		if(%client.hip == 0)
		{
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $LLeg[%client.lleg], getClosestPaintColor(%client.llegColor));
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, $RLeg[%client.rleg], getClosestPaintColor(%client.rlegColor));
		}
		else //Skirt
		{
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, "SkirtTrimLeft", getClosestPaintColor(%client.llegColor));
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 2, "SkirtTrimRight", getClosestPaintColor(%client.rlegColor));
		}
		if(%client.decalName !$= "AAA-None")
		{
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 3, %client.decalName, 0);
			%blanks++;
		}
		if(%client.faceName !$= "smiley")
		{
			serverCmdAddEvent(%client, 1, $OnBotSpawn, 0, 1, "", $EditAppearance, 4, %client.faceName, 0);
			%blanks++;
		}
...
//Find the event numbers for input "OnBotSpawn" and output "EditAppearance"
function initApplyAv()
{
	%i = -1;
	while($InputEvent_NamefxDTSBrick_[%i++] !$= "OnBotSpawn")
	{
		continue;
	}
	$OnBotSpawn = %i;
